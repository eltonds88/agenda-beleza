...
namespace AgendaBeleza.Api.Repositorios
{
    public abstract class RepositorioBase
    {
        ...
        protected DynamicParameters CreateParameters(params object?[] keyValueParameters)
        {
            var parametros = new DynamicParameters();
            if (keyValueParameters != null)
            {
                for (int idx = 0; idx < keyValueParameters.Length; idx += 2)
                {
                    parametros.Add(keyValueParameters[idx]?.ToString(), keyValueParameters[idx + 1]);
                }
            }
